...
class Main(QtWidgets.QMainWindow, Ui_MainWindow):
    def __init__(self):
        ...
        self.readyConnect = True
        self.findBtn.clicked.connect(self.timKiemPort)
        self.connectBtn.clicked.connect(self.ketNoi)
    ...
    def timKiemPort(self):
        self.boxPorts.clear()
        print ([port.portName() for port in QSerialPortInfo().availablePorts()])
        print(len(self.boxPorts.currentText()))
        self.boxPorts.addItems([ port.portName() for port in QSerialPortInfo().availablePorts() ])

    def ketNoi(self):
        print(len(self.boxPorts.currentText()))
        if self.readyConnect and len(self.boxPorts.currentText()) > 0:
            self.port.setBaudRate(115200)
            self.port.setPortName(self.boxPorts.currentText())
            self.port.open(QtCore.QIODevice.ReadWrite)
            if self.port.isOpen():
                print("đã kết nối")
                self.connectState.setText("Đã kết nối")
                self.sendFromPort("n")
                self.readyConnect = False
                self.boxPorts.setEnabled(False)
                self.findBtn.setEnabled(False)
                self.btns.setEnabled(True)

            else:
                print("chưa kết nối")
                self.connectState.setText("Chưa kết nối")
        else:
            self.port.close()
            self.btns.setEnabled(False)
            self.connectState.setText("Chưa kết nối")
            ...
            self.TrangThaiCong.setText("CHẾ ĐỘ CỔNG")
            self.maThe.setText("")
            self.maxXe.setText("")
            self.soXe.setText("")
            self.ThongBao.setText(".........")
            self.ThongBao2.setText("")
            self.boxPorts.setEnabled(True)
            self.btns.setEnabled(False)
    ...
    def readFromPort(self):
        ...
        if len(self.data) > 0:
            ...
            if (inra.find("sx")>=0) and (inra.find("$", inra.find("sx")) > 0):
                soxe = inra[inra.find("sx")+2:inra.find("$", inra.find("sx"))]
                self.soXe.setText(soxe)
                ref.update({"soxe": soxe})
                print("soxe la " + soxe)
                self.data.clear()

            if (inra.find("mx")>=0) and (inra.find("$", inra.find("mx")) > 0):
                max = inra[inra.find("mx")+2:inra.find("$", inra.find("mx"))]
                self.maxXe.setText(max)
                ref.update({"max": max})
                print("max la " + max)
                self.data.clear()
            
            if (inra.find("cc")>=0) and (inra.find("$", inra.find("cc")) > 0):
                c = inra[inra.find("cc")+2:inra.find("$", inra.find("cc"))]
                if c == "1":
                    self.TrangThaiCong.setText("CỔNG VÀO")
                else:
                    self.TrangThaiCong.setText("CỔNG RA")

                print("che do cong la " + c)
                self.data.clear()
